...
def create_driver():
    ...
    c.execute("""INSERT INTO drivers (username, full_name, status, driver_type, home_base, current_location)
                 VALUES (?, ?, ?, ?, ?, ?)""",
              (data["username"], data["full_name"], data.get("status", "available"),
               data.get("driver_type", "OTR"), data.get("home_base", ""), data.get("current_location", "")))
    conn.commit()
    driver_id = c.lastrowid
    conn.close()
    return jsonify({"id": driver_id, "message": "Driver created"}), 201
...
def update_driver(driver_id):
    ...
    conn.execute(f"UPDATE drivers SET {fields} WHERE id=?", (*data.values(), driver_id))
    conn.commit()
    conn.close()
...
def create_load():
    ...
    c.execute("""INSERT INTO loads (load_number, origin, destination, pickup_date, delivery_date, weight, miles, rate, load_type, commodity)
              ...
              (data["load_number"], data["origin"], data["destination"],
               ...
               data.get("weight", 0), data.get("miles", 0), data.get("rate", 0),
               data.get("load_type", "OTR"), data.get("commodity", "")))
    conn.commit()
    load_id = c.lastrowid
    conn.close()
    return jsonify({"id": load_id, "message": "Load created"}), 201
...
def update_load(load_id):
    ...
    conn = get_db()
    fields = ", ".join([f"{k}=?" for k in data.keys()])
    conn.execute(f"UPDATE loads SET {fields} WHERE id=?", (*data.values(), load_id))
    conn.commit()
    conn.close()
...
def create_assignment():
    ...
    match_type = data.get("match_type", random.choice(match_types))
    ...
    conn.commit()
    conn.close()
    return jsonify({"id": assignment_id, "match_score": match_score, "match_type": match_type}), 201
...
def match_drivers():
    ...
    return jsonify({"load": load, "matches": matches})
...
def optimize_route():
    ...
    conn.commit()
    conn.close()
    return jsonify({"optimized": True, "savings_miles": round(route["total_miles"] - new_miles, 1), "new_total": new_miles})
